...
class LinkedListRec:
    ...
    def __init__(self, items: List) -> None:
        ...
        if len(items) == 0:
            ...
            self._first = items[0]
            # in case of only one item in the list,
            # items[1:] = []
    ...
    def __contains__(self, item: object) -> bool:
        ...
        False
        """
        # Base Case: p = 0 empty list
        if self.is_empty():
            return False
        # Base Case: p = 1
        elif item == self._first:
            return True
        else:
...
# ------------------------------------------------------------------------
    ...
    def pop_first(self) -> None:
        ...
        if self.is_empty():
            raise IndexError
        # Induction Step: len(list) > 1
        else:
            # replace the first in list by the first in rest of list
            self._first = self._rest._first
            # link the rest of list by the rest of rest
    ...
    def insert_first(self, item: object) -> None:
        ...
        '1 -> 2 -> 3'
        """
        # base case: len(self) == 0
        if self.is_empty():
            # point the first item to be item
            self._first = item
    ...
    def map(self, f: Callable[[object], object]) -> 'LinkedListRec':
        ...
        if self.is_empty():
            return newll
        elif self._rest == newll:
            newll._first = f(self._first)
            return newll
        else:
            newll._first = f(self._first)
            newll._rest = (self._rest.map(f))
            return newll
