...
class LinkedList:
    ...
    def __init__(self, items: list) -> None:  # Student(), LinkedList()
    ...
    def __getitem__(self, index: int) -> Union[object, 'LinkedList']:  #[index]
    ...
    def append(self, item: object) -> None:
        ...
        while curr is not None:
            curr = curr.next
        new_node = _Node(item)
        curr.next = new_node  # can we use _None(item) directlly here?
    ...
    def extend(self, items: list) -> None:
        ...
        '[1 -> 2 -> 3 -> 4 -> 5 -> 6 -> 7]'
        """
        new_ll = LinkedList(items)
        count = 0
        curr = self._first
        while count < len(self) - 1:
            count += 1
            curr = curr.next
        curr.next = new_ll._first
    ...
    ###########################################################################
    def __iter__(self) -> Iterator['LinkedList']:    # for i in L
...
if __name__ == '__main__':
    linky = LinkedList(['cat', 'dag', 'frog', 'duck'])
    print(linky)
