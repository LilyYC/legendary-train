...
def tile_with_dominoes(n: int) -> List['Domino']:
    ...
    **IMPORTANT!!!**
    ...
    if n == 1:
        ...
        for dominoe in upper_right_tiling:
            dominoe.add_offset(2 ** (n - 1), 0)
        for dominoe in lower_left_tiling:
            dominoe.add_offset(0, 2 ** (n - 1))
        for dominoe in lower_right_tiling:
            dominoe.add_offset(2 ** (n - 1), 2 ** (n - 1))
        # TODO (3)
        return upper_left_tiling + upper_right_tiling +lower_right_tiling +lower_left_tiling
...
def _tile_2_by_2() -> List['Domino']:
    ...
    # Remember that the positions here do *not* depend on SQUARE_SIZE.
    result = []
    domino_1, domino_2 = Domino((0, 0), (0, 1)), Domino((1, 0), (1, 1))
    domino_3, domino_4 = Domino((0, 0), (1, 0)), Domino((0, 1), (1, 1))
    direction = random.randint(0, 1)
    if direction == 1:
        result.append(domino_1)
        result.append(domino_2)
    else:
        result.append(domino_3)
        result.append(domino_4)
    return result
