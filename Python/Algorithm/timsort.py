...
def find_runs(lst: list) -> List[Tuple[int, int]]:
    ...
    while run_end < len(lst):
        ...
        #   (When you do, update runs, run_start, and run_end.)
            ...
            run_start, run_end = run_end, run_end + 1
    runs.append((run_start, run_end))
        # run_end == len(lst)
    return runs
...
def timsort(lst: list) -> None:
    ...
    while len(runs) > 1:
        top1 = runs.pop()
        top2 = runs.pop()
        _merge(lst, top2[0], top1[0], top1[1])
        new_run = (top2[0], top1[1])
        runs.append(new_run)
    ...
    while run_end < len(lst):
        ...
        if asc_flag is None:
            if lst[run_end - 1] <= lst[run_end]:
                ...
                asc_flag = False
            run_end += 1
        elif lst[run_end - 1] <= lst[run_end] and asc_flag is True:
            run_end += 1
        elif lst[run_end - 1] > lst[run_end] and asc_flag is False:
            run_end += 1
        else:
            runs.append((run_start, run_end))
            if asc_flag is False:
                lst[run_start:run_end].reverse()
            run_start, run_end = run_end, run_end + 1
            asc_flag = None
    runs.append((run_start, run_end))
    if asc_flag is False:
